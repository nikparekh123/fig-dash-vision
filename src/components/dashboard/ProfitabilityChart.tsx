...
const yearlyData = [
  { name: "FY2025", netIncome: -1250, ebitdaMargin: -120.7 },
  { name: "FY2026E", netIncome: null, ebitdaMargin: null },
];
      ...
      {payload.map((p: any) => (
        <p key={p.dataKey} className="text-muted-foreground">
          {p.name}: <span className="font-mono text-foreground">
            {p.dataKey === "ebitdaMargin" ? `${p.value}%` : `$${Math.abs(p.value) >= 1000 ? `${(p.value / 1000).toFixed(2)}B` : `${p.value}M`}`}
          </span>
        </p>
      ))}
    </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {view === "quarterly" ? "FY2025 quarterly net income & EBITDA margin" : "Net income/loss, EBITDA margin & projected EPS"}
            </p>
          </div>
